...
- a domain that you can config the DNS. It could be a sub-domain. In the rest of the doc, let's say it's `noneinfo01.kr` for the email and `app.noneinfo01.kr` for SimpleLogin webapp. Please make sure to replace these values by your domain name whenever they appear in the doc. A trick we use is to download this README file on your computer and replace all `noneinfo01.kr` occurrences by your domain.
...
#### MX record
Create a **MX record** that points `noneinfo01.kr.` to `app.noneinfo01.kr.` with priority 10.

To verify if the DNS works, the following command

```bash
dig @1.1.1.1 noneinfo01.kr mx
```

should return:

```
noneinfo01.kr.	3600	IN	MX	10 app.noneinfo01.kr.
```

#### A record
An **A record** that points `app.noneinfo01.kr.` to your server IP.
If you are using CloudFlare, we recommend to disable the "Proxy" option.
To verify, the following command

```bash
dig @1.1.1.1 app.noneinfo01.kr a
```

should return your server IP.

#### DKIM
Set up DKIM by adding a TXT record for `dkim._domainkey.noneinfo01.kr.` with the following value:
...
```bash
dig @1.1.1.1 dkim._domainkey.noneinfo01.kr txt
```
...
Add a TXT record for `noneinfo01.kr.` with the value:

```
v=spf1 mx ~all
```

What it means is only your server can send email with `@noneinfo01.kr` domain.
To verify, the following command

```bash
dig @1.1.1.1 noneinfo01.kr txt
```
...
Add a TXT record for `_dmarc.noneinfo01.kr.` with the following value
...
```bash
dig @1.1.1.1 _dmarc.noneinfo01.kr txt
```
...
Replace `/etc/postfix/main.cf` with the following content. Make sure to replace `noneinfo01.kr` by your domain.
...
mydestination =
myhostname = app.noneinfo01.kr
mydomain = noneinfo01.kr
myorigin = noneinfo01.kr
...
Make sure that the database config is correctly set, replace `noneinfo01.kr` with your domain, update 'myuser' and 'mypassword' with your postgres credentials.
...
query = SELECT domain FROM custom_domain WHERE domain='%s' AND verified=true
    UNION SELECT '%s' WHERE '%s' = 'noneinfo01.kr' LIMIT 1;
```

Create the `/etc/postfix/pgsql-transport-maps.cf` file with the following content.
Again, make sure that the database config is correctly set, replace `noneinfo01.kr` with your domain, update 'myuser' and 'mypassword' with your postgres credentials.
...
query = SELECT 'smtp:127.0.0.1:20381' FROM custom_domain WHERE domain = '%s' AND verified=true
    UNION SELECT 'smtp:127.0.0.1:20381' WHERE '%s' = 'noneinfo01.kr' LIMIT 1;
```
...
- replace `noneinfo01.kr` by your domain,
...
```.env
# WebApp URL
URL=http://app.noneinfo01.kr

# domain used to create alias
EMAIL_DOMAIN=noneinfo01.kr

# transactional email is sent from this email address
SUPPORT_EMAIL=support@noneinfo01.kr

# custom domain needs to point to these MX servers
EMAIL_SERVERS_WITH_PRIORITY=[(10, "app.noneinfo01.kr.")]
...
Install Nginx and make sure to replace `noneinfo01.kr` by your domain
...
If all the above steps are successful, open http://app.noneinfo01.kr/ and create your first account!
